...
class ajax_github extends AWS_CONTROLLER
{
    ...
    public function setup()
    {
        ...
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('本站未开通 Github 登录')));
        }
    }
    ...
    public function register_action()
    {
        ...
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('Github 账号信息不存在')));
        }

        if ($this->model('openid_github')->get_github_user_by_id(AWS_APP::session()->github_user['id']))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('此 Github 账号已被绑定')));
        }
        ...
        else
        {
            ...
            if (AWS_APP::session()->github_user['email'] == $_POST['email'] AND AWS_APP::session()->github_user['verified'] == true)
            {
        ...
        $this->model('openid_github')->bind_account(AWS_APP::session()->github_user, $uid);

        if (AWS_APP::session()->github_user['picture'])
        {
            $this->model('account')->associate_remote_avatar($uid, AWS_APP::session()->github_user['picture']);
        }
